...
export interface SeedClient {
  resetDatabase: () => Promise<EndpointResponse>;
  seedCatalog: () => Promise<EndpointResponse>;
  seedTestRepository: () => Promise<EndpointResponse>;
  seedUser: () => Promise<EndpointResponse>;
}

export const getSeedClient = async (baseUrl: string): Promise<SeedClient> => {
  const ENDPOINT_URL = new URL('/api/seed/', baseUrl);
  const req = await Playwright.request.newContext();
  const defaultUser = userSeed[0];
  ...
  const getSeedUrl = (path) => new URL(path, ENDPOINT_URL).toString();

  async function resetDatabase() {
    const res = await req.post(getSeedUrl('reset'));
    return formatResponse(res);
  }

  async function seedCatalog() {
    const res = await req.post(getSeedUrl('catalog'));
    return formatResponse(res);
  }

  async function seedTestRepository() {
    const res = await req.post(getSeedUrl('repository'));
    return formatResponse(res);
  }

  async function seedUser() {
    const res = await req.post(getSeedUrl('user'));
    return formatResponse(res);
  }

  return { resetDatabase, seedCatalog, seedTestRepository, seedUser };
};
