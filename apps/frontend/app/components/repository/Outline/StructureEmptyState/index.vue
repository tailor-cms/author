<template>
  <TailorEmptyState
    v-bind="$attrs"
    height="auto"
    text="Start building the outline — create an item from scratch, or bring
      one in from another repository."
    text-width="440"
    title="No items yet"
    variant="text"
  >
...
// Copying at the root offers the schema's root-level activity types.
const { sameLevel } = useSelectedActivity(null);
const rootLevels = computed(() =>
  sameLevel.value.map((it: any) => it.type),
);
...
const openRenoir = () =>
  $eventBus.channel('agent').emit('prompt:set', {
    prompt: 'Draft an initial outline for this repository.',
  });
    ...
    ? [
        {
          ...
          text: 'Have the AI agent draft your outline.',
