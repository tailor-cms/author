      ...
      <div class="primary-darken-3 elevation-2">
        <VTabs
          ...
          <VTab
            ...
            class="px-8">
            <VIcon start class="text-primary-lighten-1">
              mdi-{{ tab.icon }}
            </VIcon>
            {{ tab.name }}
          </VTab>
...
const tabs = computed(() => {
  const hasSettingsAvailable = !!currentRepositoryStore.repository?.hasAdminAccess;
  const hasActivities = !!currentRepositoryStore.activities.length;
  return getTabItems({ hasSettingsAvailable, hasWorkflow: false, hasActivities, query: {} });
});
</script>
