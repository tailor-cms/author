...
const MAPPINGS = [
  ...
  [
    'repository_user',
    'course_user_course_id_fkey',
    'repository_user_repository_id_fkey',
  ],
  [
    'repository_user',
    'course_user_user_id_fkey',
    'repository_user_user_id_fkey',
  ],
];
...
['activity', 'comment', 'revision', 'teaching_element'].forEach((table) => {
  MAPPINGS.push([
    table,
    `${table}_course_id_fkey`,
    `${table}_repository_id_fkey`,
  ]);
});
...
exports.down = (queryInterface) => {
  const { sequelize: db } = queryInterface;
  return Promise.each(MAPPINGS, (it) =>
    updateConstraint(db, [it[0], it[2], it[1]]),
  );
};
